...
type AppState = {
    ...
    updateKey: number; // Used to force re-render when switching views so componentDidMount runs again
...
export default class App extends Component<{}, AppState> {
    ...
    render() {
        if (this.state.currentView === 'PUBLIC') {
            return (
                <div className="min-h-screen bg-[#1F1F21] text-zinc-100 font-sans selection:bg-red-600 selection:text-white flex flex-col">
                    ...
                    <footer className="border-t border-zinc-800 bg-[#18181A] py-8 mt-12 text-center text-zinc-400 text-xs sm:text-sm">
                        <p>FelipCar Estética Automotiva © 2026. Todos os direitos reservados.</p>
                    </footer>
                </div>
            );
        }

        return <AdminDashboard onLogout={() => this.switchView('PUBLIC')} />;
    }
}
